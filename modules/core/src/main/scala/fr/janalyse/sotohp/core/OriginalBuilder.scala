...
object OriginalBuilder {
  ...
  def originalFromFile(
    ...
    for {
      ...
      orientation       = drewMetadata.flatMap(extractOrientation)
      aperture          = drewMetadata.flatMap(extractAperture)
      shutterSpeed      = drewMetadata.flatMap(extractExposureTime)
      iso               = drewMetadata.flatMap(extractISO)
      focalLength       = drewMetadata.flatMap(extractFocalLength)
      location          = drewMetadata.flatMap(extractLocation)
