...
// Build a cropped-or-full thumbnail. If asset.selectedBox is set, returns a wrapper
// that displays only the crop region scaled to fill the container.
// Always uses the normalized image as the source.
...
async function renderPortfolioMosaic(container, assets) {
  ...
  // Build grid: 1 = single full-bleed, 2 = two columns, 3 = top spans 2 + 2 cells below, 4 = 2x2
...
function openAssetEditModal(portfolioId, asset) {
  ...
  function onMoveStart(e) {
    if (e.target !== overlayEl) return; // handle clicks handled separately
    e.preventDefault(); e.stopPropagation();
    const rect = img.getBoundingClientRect();
